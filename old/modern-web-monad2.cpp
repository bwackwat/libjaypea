...
static enum RequestResult parse_request(const char* request, JsonObject* request_obj){
	std::string new_key = "";
...
int main(int argc, char **argv){
		...
		server.on_read = [&](int fd, const char* data, ssize_t data_length)->ssize_t
	{
		...
		enum RequestResult r_type = parse_request(data, r_obj);
		...
		if(r_type == API || r_type == OTHER){
			...
			if(routemap.count(api_route)){
				...
				if(good_call){
					response_body = routemap[api_route](r_obj);
				}
			}else{
